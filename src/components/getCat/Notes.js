...
class Notes extends Component {
  ...
  api() {
    ...
    const notesid = getdata.cat.notesid;
    axios.get('get_detail/' + notesid + "/" + id).then(res => {

      const total = res.data.data[0].book.length;
      for (let i = 0; i < total; i++) {
        this.state.notes.push(res.data.data[0].book[i])
      }
      this.setState({
        loaded: true
      })
  ...
  addtofav(id, index) {
    ...
    var c = document.getElementsByClassName('n')[index];
        ...
        <div className="wrapper">
            ...
            <div className="featured-header px-5">
              <div className="row">
                <div className="feaured-books-heading-box  py-1 w-100 px-1 col-sm-12 d-flex justify-content-betwen place-items-center">
                  <div> <h3 className="d-flex  justify-content-betwen place-items-center my-2"> <span className="brand-color "><Link style={{ textDecoration: 'none' }} to="/allnotes">Notes </Link> <BsIcons.BsChevronDoubleRight /> {this.props.match.params.name}</span>
          ...
          <div className="container ">
            ...
            <div className="books-outer-box pt-1">
              <div className={this.state.notes.length > 0 ? "cards-body" : ""}>
                {
                  this.state.notes.length > 0 ?
                    this.state.notes.map((item, i) =>
                      <div key={i} className="books-card">
                        ...
                        <div className={item.favorite === 0 ? "n add-to-cart" : "n remove-to-cart"} onClick={() => { this.addtofav(item._id, i) }}><AiIcons.AiOutlineHeart className="wish-icon" /> </div>

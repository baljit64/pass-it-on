...
class Books extends Component {
  ...
  api() {
    ...
    axios.get('get_detail/' + bookid + "/" + id).then(res => {

      const totalbooks = res.data.data[0].book.length;
      for (let i = 0; i < totalbooks; i++) {
        this.state.books.push(res.data.data[0].book[i])
      }
      this.setState({
        loaded: true
      })
  ...
  addtofav(id, index) {
    ...
    var c = document.getElementsByClassName('b')[index];
            ...
            <div className="featured-header px-5">
              <div className="row">
                <div className="feaured-books-heading-box  py-1 w-100 px-1 col-sm-12 d-flex justify-content-betwen place-items-center">
                  <div> <h3 className="d-flex  justify-content-betwen place-items-center my-2"> <span className="brand-color "><Link style={{ textDecoration: 'none' }} to="/allbooks">Books </Link> <BsIcons.BsChevronDoubleRight /> {this.props.match.params.name}</span>
                    ...
                    this.state.books.map((item, i) =>
                      <div key={i} className="books-card ">
                        ...
                        <div className="read-more-btn">
                          <Link style={{ textDecoration: 'none' }} to={"/bookdis/" + item._id}></Link>
                        </div>
                        <div className={item.favorite === 0 ? "b add-to-cart" : "b remove-to-cart"} onClick={() => { this.addtofav(item._id, i) }}><AiIcons.AiOutlineHeart className="wish-icon" /> </div>
